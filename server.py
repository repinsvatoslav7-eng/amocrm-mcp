...
class BearerAuthMiddleware:
    """Simple bearer protection for the MCP endpoint. /health stays public for Render."""

    def __init__(self, app):
        self.app = app

    async def __call__(self, scope, receive, send):
        if scope.get("type") != "http" or scope.get("path") == "/health" or ALLOW_INSECURE_MCP:
            await self.app(scope, receive, send)
            return
        headers = {k.decode("latin1").lower(): v.decode("latin1") for k, v in scope.get("headers", [])}
        auth = headers.get("authorization", "")
        x_secret = headers.get("x-mcp-secret", "")
        valid = (auth == f"Bearer {MCP_SECRET}") or (x_secret == MCP_SECRET)
        if not valid:
...
app = BearerAuthMiddleware(_inner_app)
